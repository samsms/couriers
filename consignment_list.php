...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<a class="btn btn-primary" id="continue" ><i class="fa fa-check"></i> Continue</a>
...
<script>
    ...
    $("#continue").click(function(e){
        var session_id = '<?php echo $_SESSION["system"]["id"] ?>';
       var ids=$("input[id^=selected]:checkbox:checked").map(function(){
            return $(this).val();
        });
       if(ids.get()==""){
           ...
           $.ajax({
			url:'ajax.php?action=transit_consignments',
			method:'POST',
			data: {ids: ids.get(), user: session_id},
			error:err=>{
				console.log(err)
			...
			success:function(resp){
			    console.log(resp);
			    if(resp == 1){
			        alert_toast("Selected consignments moved to transit",'success')
			        setTimeout(function(){
                      location.reload(); 
                    },1000)
			    }else{
	...
	function fetchConsignments(){
	     $.ajax({
			url:'ajax.php?action=get_consignments',
			method:'POST',
			data: null,
			error:err=>{
				console.log(err)
				...
				if(typeof resp === 'object' || Array.isArray(resp) || typeof JSON.parse(resp) === 'object'){
					...
					if(Object.keys(response).length > 0){
						...
						Object.keys(response).map(function(k){
							...
							var classes = (response[k].lm == '1') ? "btn btn-success" : "btn btn-warning";
							var classes1 = (response[k].s == '1') ? "btn btn-success" : "btn btn-warning";
							
							var td1 = $('<td></td>')
							var div1 = $('<div class="btn-group"></div>')
							div1.append('<button type="button" class="btn-flat '+classes+'" >LM</button>')
							div1.append('<button type="button" class="btn-flat '+classes1+'">S</button>')
	...
	function approve_parcel(id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Data successfully approved",'success')
					setTimeout(function(){
						location.reload()
					},1500)
