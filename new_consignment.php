	...
	<div class="card card-outline card-primary">
        ...
        <div class="row">
          ...
          <div class="col-md-6">
              ...
              <div class="form-group">
                <label for="" class="control-label">Name</label>
                <!--<select name="customer_id" id="vehicle_id" class="form-control select2">
                <option value=""></option>
                <?php 
                  $customers = $conn->query("SELECT * FROM customer");
                    while($row5 = $customers->fetch_assoc()):
                ?>
                  <option value="<?php echo $row5['id'] ?>" <?php echo isset($recipient_name) && $recipient_name == $row5['id'] ? "selected":'' ?>><?php echo ucwords($row5['name']) ?></option>
                <?php endwhile; ?>
              </select>-->
          ...
          <div class="col-md-6" id=""  <?php echo isset($type) && $type == 1 ? 'style="display: none"' : '' ?>>
            <?php if($_SESSION['login_branch_id'] <= 0): ?>
              <div class="form-group" id="fbi-field">
                <label for="" class="control-label">Shipping Warehouse</label>
            ...
            <div class="form-group" id="tbi-field">
              <label for="" class="control-label">Shipping Destination</label>
            ...
            <div class="form-group" id="tbii-field">
              <label for="" class="control-label">Vehicle Information</label>
              <select name="vehicle_id" id="vehicle_id" class="form-control select2">
                <option value=""></option>
                <?php 
                  $vehicles = $conn->query("SELECT * FROM vehicle");
                    while($row2 = $vehicles->fetch_assoc()):
                ?>
                  <option value="<?php echo $row2['id'] ?>" <?php echo isset($vehicle_id) && $vehicle_id == $row2['id'] ? "selected":'' ?>><?php echo ucwords($row2['plate']) ?></option>
                <?php endwhile; ?>
              </select>
            </div>
        ...
        <table class="table table-bordered" id="parcel-items">
          ...
          <tbody>
            <tr>
              <td><input type="number" name='weight[]' value="<?php echo isset($weight) ? $weight :'' ?>" required></td>
              ...
              <input type="hidden" name='width[]' value="<?php echo isset($width) ? $width :'20' ?>" required>
              <input type="hidden" class="text-right number" name='price[]' value="<?php echo isset($price) ? $price :'100' ?>" required>
            </tr>
          </tbody>
        </table>
              <?php if(!isset($id)): ?>
...
<div id="ptr_clone" class="d-none">
  <table>
    <tr>
        ...
        <td><input type="hidden" name='width[]' required></td>
        <td><input type="hidden" class="text-right number" name='price[]' required></td>
      </tr>
  </table>
</div>
<script>
    ...
    if($('#parcel-items tbody tr').length <= 0){
      ...
      //       }
        ...
        if(resp == 1){
            alert_toast('Data successfully saved',"success");
            setTimeout(function(){
              location.href = 'index.php?page=consignment_list';
            },2000)
