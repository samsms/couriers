			...
			<div class="d-flex w-100 px-1 py-2 justify-content-center align-items-center">
				...
				<select name="" id="status" class="custom-select custom-select-sm col-sm-3">
					...
					while($row= $query->fetch_assoc()): ?>
						<option value="<?php echo $row['id'] ?>" <?php echo $status != 'all' && $status == $row['id'] ? "selected" :'' ?>><?php echo $row['name']; ?> Stage</option>
					<?php endwhile; ?>
				</select>
					...
					if(typeof resp === 'object' || Array.isArray(resp) || typeof JSON.parse(resp) === 'object'){
						...
						if(Object.keys(resp).length > 0){
							...
							Object.keys(resp).map(function(k){
								...
								tr.append('<td>'+(resp[k].reference_number)+'</td>')
								tr.append('<td>KDG 146T</td>')
								tr.append('<td>'+(resp[k].recipient_name)+'</td>')
								tr.append('<td>'+(resp[k].length)+'</td>')
								tr.append('<td>'+(resp[k].street)+'</td>')
